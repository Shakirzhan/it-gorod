...
//Ïàò÷èì ôàéëû äëÿ ïðàâèëüíîé ðàáîòû ñêðèïòà ñêðûòèÿ ìîäóëåé
echo not_mine();
...
//Óñòàíàâëèâàåì çàãîëîâîê ñòðàíèöû
$APPLICATION->SetTitle("Get Update Modules ver 2.3.0 (beta)");

/////////////////////
//Íà÷èíàåì

//Åñëè êëþ÷ ìåíüøåé äëèííû èëè íàæàòà êíîïêà Ñáðîñèòü
if(strlen($_POST["k"])<23 || $_POST["command"]=="reset" || $_SESSION['k'] != $_POST["k"]){
	clear();	
}
if($_SESSION['k'] != $_POST["k"]) getKeyInfo(TRUE);

//ñòðîèì îñíîâíîé ìàññèâ ñ äàííûìè
rebuild();


//Åñëè íàæàòà êíîïêà ïîëó÷èòü èíôó î ìîäóëÿõ
if($_POST["command"]=="module"){
	
	//Åñëè êëþ÷ íå èçìåíèëñÿ
	if($_SESSION['k'] == $_POST["k"]){
		echo KEYNOTCHANGE;
	}
	//Åñëè êëþ÷ êîðî÷å ÷åì äîëæåí áûòü
	elseif(strlen($_POST["k"])<23){
		echo SHORTKEY;
	}
	//Åñëè ïåðåäàí íîâûé êëþ÷ è íóæíîé äëèííû
	else{
		unset($_SESSION['KeyInfo']['ERROR']);
		//Ñîõðàíÿåì êëþ÷ â ñåññèè
		$_SESSION['k'] = $_POST["k"];
		echo MADEREQUEST;
		//Ïîëó÷àåì èíôîðìàöèþ î êëþ÷å è ñîõðàíÿåì â ñåññèþ
		getKeyInfo();
...
//Ñîðòèðóåì ìàññèâ ñ ìîäóëÿìè
sortArray($_SESSION['KeyInfo']['MODULES']);

// Âûâîäèì îøèáêó åñëè åñòü
echo "<p>".$_SESSION['KeyInfo']['ERROR']."</p>";

//ññûëêè äëÿ óìåíüøåíèÿ êîäà
...
<p>Êëþ÷: <input id="key" name="k" form="infkey" value="<?=htmlspecialchars($_POST['k'])?>" size="28" maxlength="23"></p>


<p><input name="command" form="infkey" value="module" hidden="true">
<input type="submit" form="infkey" value="Ïîëó÷èòü èíôîðìàöèþ î ìîäóëÿõ">

<input name="command" form="reset" value="reset" hidden="true">
<input type="submit" form="reset" value="Ñáðîñèòü"></p> 

<?//Ñòðîèì âûâîä èñõîäÿ èç ïîëó÷åííîé èíôîðìàöèè?>

<form id="downmod" action="get_update.php" method="post"></form>
<div id="updater">
<?//Ïîëó÷åíà ëè èíôîðìàöèÿ î êëþ÷å
	

	if(is_array($_SESSION['KeyInfo']['CLIENT'])){
		echo "<div id='keyinfo'><p>Çàðåãèñòðèðîâàíî íà èìÿ \"".$cName."\", îáíîâëåíèÿ äîñòóïíû ñ ".$dateFrom." ïî ".$dateTo."</p>";
		if(count($modules)>0) echo '<br><p>Ñïèñîê ìîäóëåé:</p>';
		echo "</div>";

	}

	//Ïðîáåãàåìñÿ ïî âñåì ìîäóëÿì è âûâîäèì ñòðîêè
	foreach($modules as $modId => $infMod){
		//Ïðîâåðÿåì åñëè åñòü óñòàíîâëåííûé ìîäóëü èëè äîñòóïíî îáíîâëåíèå
...
$(document).on('click', '.ajax-send-dwl',function(){
	...
	$.ajax({
        ...
        complete: function(data){
		   ...
		   if(inf==1){
		   	    $("#"+controlInf+"-inf").append("<span class='green'>Óñïåøíî ñêà÷àí ôàéë!</span>");
	           	...
	           	$("#"+controlInf+"-inf").append("<span class='red'>Ïðîèçîøëà íåèçâåñòíàÿ Îøèáêà!</span>");
	           	$("#"+controlInf+"-inf span").delay(2000).fadeOut(1000, function(){$(this).remove()});
	           	$("#"+controlId).replaceWith("<div id='"+controlId+"' class='control'>"+buff+"</div>");
           }else if(inf<0){
	           	$("#"+controlInf+"-inf").append("<span class='blue'>Ôàéë óæå ïðèñóòñòâóåò â ïàïêå!</span>");
	           	$("#"+controlInf+"-inf span").delay(2000).fadeOut(1000, function(){$(this).remove()});
			   	$("#"+controlId).replaceWith("<div id='"+controlId+"' class='control'>"+buff+"</div>");
           }
...
$(document).on('click', '.ajax-send-upd',function(){
	...
	$.ajax({
        ...
        complete: function(data){
		   ...
		   if(inf==1){
		   	    $("#"+controlInf+"-inf").append("<span class='green'>Óñïåøíî óñòàíîâëåíî!</span>");
	           	$("#"+controlInf+"-inf span").delay(2000).fadeOut(1000, function(){$(this).remove()});
		   		if(type =='mod'){
...
.tooltip::after {
	...
	color: #fff;
	content: attr(data-tooltip); /* Ãëàâíàÿ ÷àñòü êîäà, îïðåäåëÿþùàÿ ñîäåðæèìîå âñïëûâàþùåé ïîäñêàçêè */
	margin-top: -24px;
	opacity: 0; /* Íàø ýëåìåíò ïðîçðà÷åí... */
	padding: 3px 7px;
	position: absolute;
	visibility: hidden; /* ...è ñêðûò. */
			
	transition: all 0.2s ease-in-out; /* Äîáàâèòü ïëàâíîñòè ïî âêóñó */
}
		
.tooltip:hover::after {
	opacity: 1; /* Ïîêàçûâàåì åãî */
	visibility: visible;
}
