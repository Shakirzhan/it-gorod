...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation)))
{
    ...
    )
    {
        ...
        $arIDElements = $arrFilter["ID"];
        $arKeyToId = array_flip( $arIDElements );
        $relativ = true;
    }
    //EXECUTE
    ...
    if ( $relativ && $arKeyToId)
    {
        $arNewItems = array();
        foreach ( $arKeyToId as $kID )
        {
            if ( $arResult["ITEMS"][$kID] )
            {
                $arNewItems[$kID] = $arResult["ITEMS"][$kID];
            }
            unset($arResult["ITEMS"][$kID]);
        }
        if ( !empty($arResult["ITEMS"]) )
        {
            $arNewItems += $arResult["ITEMS"];
        }
        ...
        if(!empty($arOffers))
        {
            ...
            foreach($arOffers as $arOffer)
            {
                if (isset($arElementLink[$arOffer["LINK_ELEMENT_ID"]]))
                {
                    ...
                    if ( !$arElementLink[$arOffer["LINK_ELEMENT_ID"]]["PREVIEW_PICTURE"] && $arOffer["PREVIEW_PICTURE"] )
                    {
                        $arElementLink[$arOffer["LINK_ELEMENT_ID"]]["PREVIEW_PICTURE"] = $arOffer["PREVIEW_PICTURE"];
                    }
                    elseif( !$arElementLink[$arOffer["LINK_ELEMENT_ID"]]["DETAIL_PICTURE"] && $arOffer["DETAIL_PICTURE"] )
                    {
                        $arElementLink[$arOffer["LINK_ELEMENT_ID"]]["DETAIL_PICTURE"] = $arOffer["DETAIL_PICTURE"];
                    }
