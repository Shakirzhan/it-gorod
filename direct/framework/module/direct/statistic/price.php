...
final class Price extends \FrameWork\Common {
	...
	public function get($PARAM=array(), $ORDER=array('id'), $FIELDS=array(), $LIMIT=array('page'=>0, 'number'=>0), $GROUP=array()) {
		...
		if (!empty($PARAM['datetime']))
			$WHERE[]="`t1`.`datetime`>='".date('Y-m-d H:i:s', strtotime($PARAM['datetime']))."'";
			
		if (!empty($PARAM['date']))
			$WHERE[]="(`t1`.`datetime`>='".date('Y-m-d', strtotime($PARAM['date']))." 00:00:00' AND `t1`.`datetime`<='".date('Y-m-d', strtotime($PARAM['date']))." 23:59:59')";
		
		if (!empty($PARAM['date_start']))
			$WHERE[]="`t1`.`datetime`>='".date('Y-m-d', strtotime($PARAM['date_start']))." 00:00:00'";
		
		if (!empty($PARAM['date_end']))
			$WHERE[]="`t1`.`datetime`<='".date('Y-m-d', strtotime($PARAM['date_end']))." 23:59:59'";
