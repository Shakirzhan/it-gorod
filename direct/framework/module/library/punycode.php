...
/// Name: Класс для работы с кириллическими доменами      ///
...
final class Punycode {
    ...
    public function __construct($options = false)
    {
		...
		if (empty($options))
			$options=array('idn_version'=>2008);
	...
	function get ($punycode='') {
		if (!empty($punycode)) {
			$punycode=(string)$punycode;
			$punycode=(stripos($punycode, 'xn--')!==false) ? $this->decode($punycode) : (mb_eregi('[а-я]', $punycode)?$this->encode($punycode):$punycode);
		}
		return $punycode;
	}
