...
final class Group extends \FrameWork\Common {
	...
	public function set($PARAMS=array()) {
		...
		$id=$this->Framework->library->model->set($this->Framework->dictionary->model->config->TABLE['group'], $PARAMS);
		if ($id) {
	...
	public function get($PARAMS=array(), $ORDER=array('id'), $FIELDS=array(), $LIMIT=array('page'=>0, 'number'=>0), $GROUP=array()) {
		...
		if (!empty($PARAMS['id'])) {
			if (!is_numeric($PARAMS['id'])) {
				$ID=$this->Framework->library->model->get($this->Framework->dictionary->model->config->TABLE['group'], array('key'=>$PARAMS['id']));
				if (!empty($ID[0]['id']))
					$PARAMS['id']=$ID[0]['id'];
			}
		...
		$sql="SELECT COUNT(*) as `count`
					FROM `".$this->Framework->dictionary->model->config->TABLE['group']."` `t1`
					{$where}
		";
