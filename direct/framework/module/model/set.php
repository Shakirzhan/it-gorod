...
final class Table extends \FrameWork\Common {
	...
	public function set($PARAM=array()) {
		if (!empty($PARAM) && is_array($PARAM)) {
			...
			elseif (isset($PARAM['status']))
				$PARAM['status']=0;
			$this->Framework->library->model->set($this->Framework->model->config->TABLE['table'], $PARAM);
		}
	}
	...
	public function get($PARAM=array(), $ORDER=array('id'), $FIELDS=array(), $LIMIT=array('page'=>0, 'number'=>0), $GROUP=array()) {
		$DATA=array();
		$WHERE=array();
		$table=$this->Framework->model->config->TABLE['table'];
