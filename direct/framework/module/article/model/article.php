...
/// Charset: UTF-8                                        ///
/////////////////////////////////////////////////////////////
namespace FrameWork\module\article\model;
...
final class Article extends \FrameWork\Common {
	...
	public function __construct () {
		parent::__construct();
		$this->Framework->library->model()->table($this->TABLES);
	}
	...
	public function set($PARAMS=array()) {
		...
		$id=$this->Framework->library->model->set($this->TABLES['article'], $PARAMS);
		if ($id) {
	...
	public function get($PARAMS=array(), $ORDER=array('id'), $FIELDS=array(), $LIMIT=array('page'=>0, 'number'=>0)) {
		...
		if (!empty($PARAMS['id'])) {
			if (!is_numeric($PARAMS['id'])) {
				$ID=$this->Framework->library->model->get($this->TABLES[0], array('key'=>$PARAMS['id']));
				if (!empty($ID[0]['id']))
					$PARAMS['id']=$ID[0]['id'];
			}
		...
		if (!empty($PARAMS['parent'])) {
			if (!is_numeric($PARAMS['parent'])) {
				$PARENT=$this->Framework->library->model->get($this->TABLES[0], array('key'=>$PARAMS['parent']));
