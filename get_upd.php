...
if($_REQUEST['action']=='dwl'){
	...
	elseif($result === FALSE) echo '<div id="'.htmlspecialchars($id).'-inf" class="0"></div>';
	elseif($result === TRUE) echo '<div id="'.htmlspecialchars($id).'-inf" class="-1"></div>';
}
...
if($_REQUEST['action']=='upd'){
	...
	if (!$uptd || !$unarch) {echo '<div id="'.htmlspecialchars($id).'-inf" class="0"></div>';}
	else {
		rebuild();
		if($request['type'] == 'mod'){
